  ...
  render() {
    ...
    return (
      <div className="graylog-input-metrics">
        <h3>吞吐量/指标</h3>
        <span>
          {isNaN(incomingMessages) && isNaN(writtenBytes1Sec) && isNaN(openConnections) && <i>这个输入没有可用的度量</i>}
          {!isNaN(incomingMessages) && <span>1分钟平均速率: {this._formatCount(incomingMessages)} msg/s<br /></span>}
          {!isNaN(writtenBytes1Sec) && this._formatNetworkStats(writtenBytes1Sec, writtenBytesTotal, readBytes1Sec, readBytesTotal)}
          {!isNaN(openConnections) && this._formatConnections(openConnections, totalConnections)}
          {!isNaN(emptyMessages) && <span>丢弃空消息: {this._formatCount(emptyMessages)}<br /></span>}
